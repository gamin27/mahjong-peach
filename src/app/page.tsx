    ...
    const fetchData = async () => {
      ...
      if (myScores && myScores.length > 0) {
        ...
        const [gamesRes, allScoresRes, tobashiRes, yakumanRes] =
          await Promise.all([
            supabase.from("games").select("id, created_at").in("id", gameIds),
            supabase
              .from("game_scores")
        ...
        for (const pc of [3, 4] as const) {
          const ids = gameIds.filter((id) => gamePlayerCount[id] === pc);
    ...
    >
      ...
      <Main maxWidth="5xl">
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg, var(--arcoblue-6), var(--arcoblue-5))",
        ...
        {!loading &&
          ...
          (() => {
            ...
            const toX = (i: number) =>
              padL + (count > 1 ? (i / (count - 1)) * chartW : chartW / 2);
            const toY = (rank: number) =>
              padT + ((rank - 1) / (maxRank - 1)) * chartH;
            const points = data
              .map((p, i) => `${toX(i)},${toY(p.rank)}`)
              .join(" ");
